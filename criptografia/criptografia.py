...
def inversa_matriz_2x2(matriz: list, mod: int):
    """
    Calcula a matriz inversa da matriz 2x2.
    ...
    Returns:
        list: A matriz inversa 2x2 em mod.
...
def descriptografar_cpf(cpf_cifrado: str):
    ...
    """
    texto = descriptografar(cpf_cifrado)    
    return texto[:-1]
...
def descriptografar_chave_acesso(chave_cifrada: str):
    ...
    """
    texto = descriptografar(chave_cifrada)

    if len(texto) % 2 == 0 and texto[-1] == texto[-2]:
        return texto[:-1]
